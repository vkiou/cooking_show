...
FROM
    cook c
    JOIN cook_specialization cs ON c.cook_id = cs.cook_id
    JOIN cuisine_by_country cb ON cs.cbc_id = cb.cbc_id
    JOIN episode_cook_participant ecp ON c.cook_id = ecp.cook_id
    JOIN episode e ON ecp.episode_id = e.episode_id
    JOIN season s ON e.season_id = s.season_id
WHERE
    cb.country = 'French'  -- replace with the actual country name
    AND s.year = 2022  -- replace with the actual year
ORDER BY
    c.cook_ID;


/*#3 
SELECT rc.cook_id, COUNT(rc.recipe_id) AS Recipe_Count
FROM cook c
JOIN cook_specialization cs ON c.cook_id = cs.cook_id
JOIN cuisine_by_country cbc ON cs.cbc_id = cbc.cbc_id
JOIN recipe rc ON cbc.cbc_id = rc.cbc_id
WHERE rc.age < 30
GROUP BY rc.cook_id
ORDER BY Recipe_Count DESC; -- desc mesns descending order*/


#4
SELECT c.cook_ID
FROM cook c
LEFT JOIN episode_cook_judge ecj ON c.cook_ID = ecj.cook_ID -- we use left join instead of join because we need the NULL
...
FROM
    ...
    JOIN season s ON e.season_id = s.season_id
WHERE
    s.year = 2020 -- specify the year you are interested in
GROUP BY
    cj.cook_id
HAVING
    COUNT(cj.episode_id) > 3;
   ...
   WITH EpisodeDifficulty AS (
    ...
    FROM 
        ...
        JOIN (
            SELECT 
                e.episode_id,
                MAX(r.difficulty_level) AS difficulty_level
            FROM 
                Episode e
                JOIN Recipe_by_country rbc ON e.Episode_ID = rbc.Recipe_ID
                JOIN Recipe r ON rbc.Recipe_ID = r.Recipe_ID
            GROUP BY 
                e.Episode_ID
        ) AS max_difficulty_per_episode ON e.Episode_ID = max_difficulty_per_episode.Episode_ID
    GROUP BY 
        e.Episode_ID, e.episode_number, e.season_year
)
SELECT 
    Episode_ID,
    episode_number,
